...
The merged company needs a unified compensation framework. The merged-company commission decisions span: commission basis (commissionable GP per CRM §3.02 D-6 — quoted labor rates; the 15% labor markup retention vs. elimination is an open working-session decision per D-1), rate structure, split mechanics, eligibility rules, payout timing, internal-referral compensation, tiered rates and bonus programs, and reporting. Both companies' inputs contribute, and several decisions belong to merged-company sales and finance leadership rather than the system itself.

**Implementation sequencing — Commission Module cut-over is post-go-live.** GSI recommends that the Commission Module be **configured and operational in NetSuite Orion at go-live**, but that the merged company **continue operating its existing commission processes in parallel** until the Module's calculations are validated against the parallel runs. Cut-over to using the Commission Module for live commission payment happens **after go-live**, once the parallel-run validation confirms the configuration is correct. This sequencing reduces risk during cutover (commission payment is high-visibility for the sales team), gives the finance team time to reconcile the merged-company commission framework against the previous KBM and Pivot processes, and provides a clean window for sales-team communication on framework changes.
...
**Recommendation for the merged company.** Operate the merged company with commissions calculating on commissionable GP per the locked CRM §3.02 D-6 framework (quoted labor rates). The Internal Services Cost Basis field on each commission plan record allows configuration of Actual vs. Quoted for cases where that flexibility is needed (per Pivot §3.02.06). The **15% labor markup retention vs. elimination is open** — KBM's existing pattern is documented and operationally embedded (Order Management REQ-038; Operations REQ-021 / REQ-022 / REQ-023; Pre-Quote v2.0 REQ-LQ-006). GSI's instinct is that the commissionable-GP framework on quoted labor renders the 15% markup formula line unnecessary, but because the pattern is KBM's and the change would materially affect the KBM sales team's compensation experience, **we follow KBM's lead on whether to retain or eliminate the markup**. The working session is where this decision lands.

**Decisions for the leadership team.**

- (1a) Confirm: commissions calculate on commissionable GP (quoted labor rates) per CRM §3.02 D-6 framework, with Internal Services Cost Basis configurable on each commission plan record. *Recommended default: yes.*
- (1b) Decide: **retain or eliminate the 15% labor markup formula-line pattern** (KBM REQ-038 / Operations REQ-021-023 / Pre-Quote v2.0 REQ-LQ-006). *Decided at working session with KBM leadership input; sales-team communication plan depends on the outcome.*
