...
- Exemplis manufacturer integration (KBM REQ-020) — separate furniture manufacturer, not MillerKnoll
...
**Pivot's approach.** Pivot's current document management is fragmented across IQ Coordinator (receiving documents), Workfront (project documents and tasks), and likely file shares (per Pivot BRD §3.12.02 / `:1291-1297`). Pivot's stated current-state pain is that "fragmentation creates challenges finding correct document versions and ensuring team access to current information." Workfront's contract expires July 2026 — a consideration for migration sequencing rather than a hard deadline; Pivot leadership has acknowledged the merged company will likely not be live by that date, and a shorter Workfront extension is part of the transition conversation. SharePoint is the **target** consolidation platform via Pivot REQ-1.12.01 through REQ-1.12.08 (centralized SharePoint integration; folder structure aligned to customer and project hierarchy; document access from NetSuite customer / project / transaction records; replace IQ and Workfront document storage by consolidating to SharePoint; role-based access; automated folder creation; version control; document templates auto-saving to SharePoint).
...
KBM users onboard into SharePoint as part of the Discovery → Activate transition. Workfront's July 2026 contract expiration is a consideration for Pivot-side document migration sequencing; a shorter Workfront extension may be negotiated if needed to align with the merged-company go-live timeline.
...
**Recommendation for the merged company.** Adopt the standard Orion 25+ custom role taxonomy with merged-company adaptations (KBM REQ-010): roles support both Pivot's divisional structure and KBM's geographic territories per the two-dimensional model locked in CRM §3.02 D-3a. Sales Locations per geography taxonomy are **configured here** in System Setup; the geographic dimension and division taxonomy are **decided in CRM §3.02 D-3a / D-3b**. Operations-specific roles reflect Pivot's primarily-internal operations team plus KBM's outsourced-operations coordination roles (per Operations §3.05). **Role naming will require merged-company-specific renaming** — KBM and Pivot use different role names for similar functions (e.g., Account Manager, Project Coordinator, IPM, Sales Coordinator, Project Manager); the merged-company role naming convention is finalized during configuration with both leadership teams' input.

**Decisions for the leadership team.**

- (4a) Confirm: standard Orion 25+ custom role taxonomy adapted for the merged-company operating model; Sales Locations and division-based role configuration align with CRM §3.02 D-3a / D-3b decisions. *Recommended default: yes.*
- (4b) Confirm: merged-company role naming convention finalized during configuration with both leadership teams' input. *Recommended default: yes; specific names decided during configuration.*
...
**KBM's approach.** KBM's source materials specify integration patterns for the MillerKnoll suite (ServiceNet, MillerKnoll Quote Tool), additional intermarket-order tooling (REQ-041), Coupa email-parsing evaluation (REQ-040 — pending ROI), Exemplis manufacturer integration (REQ-020 — separate manufacturer, not MillerKnoll), Stripe, Paylocity (with merged-company payroll provider decision pending), West Coast Community Bank, Google Drive (sunsetting per CT-14), Expensify (KBM-side; merged-company expense platform decision pending per Financial Management §3.08 D-7), and additional manufacturer expansion.
...
- MillerKnoll suite (ServiceNet, MillerKnoll Quote Tool, MillerKnoll Order Manager) — owned in Order Management §3.04 D-7; Coupa email-parsing automation evaluation deferred to Realize per Order Management §3.04 D-7b
- Exemplis manufacturer integration (KBM REQ-020 — separate furniture manufacturer, not MillerKnoll)
...
- Manufacturer integration expansion beyond MillerKnoll and Exemplis — KBM-side pending items per `BRD_SystemSetupConfiguration_v1.0.md:381-385`
...
- (6a) Confirm: combined confirmed-integrations architecture (HubSpot, banking with Advanced Electronic Bill Payments, MillerKnoll suite — ServiceNet / Quote Tool / Order Manager, Exemplis manufacturer integration, SharePoint via native connector, SuiteAnalytics Connect if licensed, Stripe) with NetSuite Orion as system of record. *Recommended default: yes.* (Technical design finalized during Realize phase.)
...
| **MillerKnoll integration suite (ServiceNet, MillerKnoll Quote Tool, MillerKnoll Order Manager)** | D-6 | **Order Management (§3.04 D-7)** — integration suite decision; technical setup here |
...
- **Document management:** NetSuite File Cabinet as primary for transactional documents; SharePoint via native connector (Pivot REQ-1.12.01 through REQ-1.12.08) as merged-company collaboration platform; KBM Google Drive content migrates to SharePoint; Pivot fragmented sources (IQ / Workfront / file shares) consolidate to SharePoint and File Cabinet by document type per CT-14; Workfront's July 2026 contract expiration is a sequencing consideration (shorter Workfront extension negotiable if needed)
...
- **Integration architecture:** Confirmed integrations (HubSpot, banking with Advanced Electronic Bill Payments, MillerKnoll suite — ServiceNet / Quote Tool / Order Manager, SharePoint, SuiteAnalytics Connect if licensed, Stripe) configured here against owning-section decisions; evaluation / licensing decisions (payroll provider, expense platform, ZoomInfo, manufacturer integration expansion including Exemplis and others) decided in owning sections, configured here once decisions land
...
| 4a | Standard Orion 25+ custom role taxonomy (KBM REQ-010) configured against CRM §3.02 D-3a / D-3b decisions | Yes | D-4 |
| 4b | Merged-company role naming convention finalized during configuration (KBM and Pivot use different role names for similar functions) | Yes; specific names decided during configuration | D-4 |
...
> 12 decisions across 6 divergences, all with default-yes recommendations. The merged-company configuration direction is committed; the working session focus is confirmation and transition-timeline detail per area. Role naming convention is a configuration decision (decision 4b), not a working-session selection.
...
| Pivot fragmented document sources (IQ / Workfront / file shares / email / local storage) consolidation to SharePoint / File Cabinet | N/A | Specified as current-state pain (`:1291`) | Consolidate per D-3c during IQ / Workfront sunset; Workfront July 2026 contract expiration is a sequencing consideration (shorter extension negotiable if needed) |
| Integration architecture — confirmed integrations | Multiple specified (ServiceNet, MillerKnoll Quote Tool, additional intermarket-order tooling, Exemplis, Stripe, West Coast Community Bank) | Multiple specified (HubSpot, Comerica, MillerKnoll Order Manager, SharePoint native connector) | Build combined per D-6a |
...
3. **Pivot fragmented document migration timeline** (decision 3c) — schedule for consolidating Pivot's IQ / Workfront / file shares / email / local storage into SharePoint / File Cabinet; Workfront's July 2026 contract expiration is a sequencing consideration (a shorter Workfront extension may be negotiated if needed to align with merged-company go-live); finalized during change-management planning in coordination with Project Management §3.06 D-1.
